...
class Game{
    ...
    *  */
     ...
     * @private
     */
    _getRoleByid(role_id){
        var role = null;
        for(let i = 0 ; i < this.roleAry.length;i++){
            if(this.roleAry[i]._id == role_id ){
                role = this.roleAry[i];
                break;
            }
        }
     ...
     * @private
     */
    _getPlayerByid(player_id){
        var player = null;
        for(let i = 0 ; i < this.playerAry.length;i++){
            if(this.playerAry[i]._id == player_id ){
                player = this.playerAry[i];
                break;
            }
        }
        return player;
     ...
     * @returns {*}
     */
    actionRole(player_id , roleIndex){
        //role execute play
        var gameRole = this._getRoleByid(roleIndex);
        //
        var player = this._getPlayerByid(player_id);
        player.currentRole = gameRole;
